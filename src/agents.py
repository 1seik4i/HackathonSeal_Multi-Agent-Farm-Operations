        ...
        for device in ALL_DEVICES:
            ...
            for metric, value in item["metrics"].items():
                evidence.append(Evidence(
                    ...
                    timestamp=item.get("timestamp"), received_at=item.get("received_at"),
                    source_type=item.get("source_type", "API"), topic=item.get("topic"),
                ))
...
class IrrigationPlanningAgent:
    ...
    def plan(self, observation: dict[str, Any]) -> dict[str, Any]:
        ...
        if soil < 35:
            ...
            return {"agent": self.name, "decision": "IRRIGATE", "reason": f"Soil moisture is low at {soil}%; temperature {temperature}°C and light data determine the irrigation window.", "schedule": {"start_time": slot, "duration_minutes": 20, "priority": "HIGH", "target_zone": "Farm Zone 1"}}
...
class FarmActionAgent:
    ...
    def create(self, irrigation: dict[str, Any], resources: dict[str, Any], evidence: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        return {"agent": self.name, "created": action.model_dump(), "verification": {"status": "PENDING", "reason": "Chưa có telemetry phản hồi từ actuator sau hành động; không đánh dấu VERIFIED."}}
